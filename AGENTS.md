...
2. **Codex 批量修正 mismatch**（见下方 prompt）
3. `export.py` — 生成训练格式

**mismatch 批量修正 prompt**（有新 mismatch 时执行）：

> 读 `~/Documents/Developer/whisper-finetune-data/manifest.jsonl` 里所有 `status=mismatch` 且没有 `review` 字段的条目，根据 `local_text` 和 `api_text` 推断最准确的转写，写回 `final_text`，`review` 标 `"ai"`。背景：普通话+英文混合语音转写，说话人是程序员，常见词汇：fine-tune、type4me、火山引擎、豆包、main 分支、finetune 分支、repo、Codex 等。两个版本都可能有错，优先修正专有名词和重复字。
